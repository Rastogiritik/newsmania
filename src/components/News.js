...
const News = (props) => {
  ...
  document.title= `${props.category} - Newsmania`;


  const updateNews = async ()=>{
  props.setProgress(0);
  const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=9566d0dce903484681cd28ead8ae39fe&page=${page}&pageSize=${props.pageSize}`;
  setLoading(true)
...
const fetchMoreData = async() => {
  setPage(page+1)
  const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=9566d0dce903484681cd28ead8ae39fe&page=${ page+1 }&pageSize=${props.pageSize}`;
  let data = await fetch(url);
  let parsedData = await data.json()
  ...
  setTotalResults(parsedData.totalResults)
    ...
    return (
      <div className="container my-2 ">
        <h1 className="text-center py-5">NewsMania - Top Headlines</h1>
        ...
        {articales.map((element)=>{
          ...
          return <div className="col-md-4" key={element.url}>
                  ...
                  imgUrl={element.urlToImage?element.urlToImage:"https://images.news18.com/ibnlive/uploads/2022/08/imran-khan-protest-twitter-166114595116x9.jpg"} 
